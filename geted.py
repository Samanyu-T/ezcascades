...
def main ():
    program_descripton = f'''
        LAMMPS Simulation script for obtaining displacement threshold energy statistics for alloys
    ...
    # recoil energy range 
    parser.add_argument("-emin", "--recoilenergy_min", type=float, default=10.0,
                        help="Minimum primary knock-on atom recoil energy in eV to sample (1 float) (default: %(default)s)")
    parser.add_argument("-emax", "--recoilenergy_max", type=float, default=100.0,
                        help="Maximum primary knock-on atom recoil energy in eV to sample (1 float) (default: %(default)s)")
    parser.add_argument("-edel", "--recoilenergy_del", type=float, default=1.0,
                        help="Primary knock-on atom recoil energy steps in eV (1 float) (default: %(default)s)")

    parser.add_argument("-ns", "--nsamples", type=int, default=100,
                        help="Number of samples per primary knock-on energy (1 int) (default: %(default)s)")

    parser.add_argument("-rpw", "--ranksperworker", type=int, default=1,
                        help="Number of MPI threads per worker for parallelisation (1 int) (default: %(default)s)")


    args = parser.parse_args()

    # split MPI comm into blocks of size rpw
    parentComm = comm 
    color = int(rank/args.ranksperworker)
    key = 0
    localComm = parentComm.Split (color, key)

    print ("GLOBAL RANK: ", rank, " COLOR: ", color)
    sys.stdout.flush ()

    comm.barrier ()

    # launch independent replicas
    replica (args, comm=localComm, color=color)

    return 0


def replica (args, comm=None, color=0):

    # fetch local rank and size of this replica
    me = comm.Get_rank()
    nprocs = comm.Get_size()
    ...
    emin = args.recoilenergy_min
    emax = args.recoilenergy_max
    edel = args.recoilenergy_del
    ...
    for _eVPKA in np.arange(emin, emax+1e-3, edel):
        for _ni in range(nsamples): 
            ...
            if (me == 0):
                ...
                kickvelocity = np.sqrt(2.*_eVPKA/(masses[str(_type)] * 1.03499e-4))
            ...
            announce("Wigner-Seitz analysis found: n_vacancies, n_interstitials\n%d %d" % (nvac, nint))

            rdirection = vvec/np.sqrt(vvec@vvec)
            lmp.command("print '%8.4f %12.8f %12.8f %12.8f %3d' append %s/log/%s.log" % (_eVPKA, rdirection[0], rdirection[1], rdirection[2], nvac, simdir, job_name))
            lmp.close()
